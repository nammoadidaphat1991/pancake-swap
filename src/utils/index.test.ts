...
import { TokenAmount, Token, ChainId, Percent, JSBI } from 'easybakeswap-sdk'
...
describe('utils', () => {
  describe('#getErcScanLink', () => {
    it('correct for tx', () => {
      expect(getErcScanLink(1, 'abc', 'transaction')).toEqual('https://bscscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getErcScanLink(1, 'abc', 'token')).toEqual('https://bscscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getErcScanLink(1, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getErcScanLink(2, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('ropsten', () => {
      expect(getErcScanLink(3, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('enum', () => {
      expect(getErcScanLink(ChainId.MAINNET, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
  })
